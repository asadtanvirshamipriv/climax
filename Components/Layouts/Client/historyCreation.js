import { createHistoryFields, createHistoryCompanies, createHistoryCheck, createHistorySelect } from '../../../functions/historyChecks';
function createHistory(Representatives, oldRecord, data, company){
    let htmlString = '';
    ...
    htmlString=htmlString+createHistorySelect(Representatives[1].records, oldRecord.docRepresentatorId, data.docRepresentatorId, 'Doc Rep');
    htmlString=htmlString+createHistorySelect(Representatives[2].records, oldRecord.salesRepresentatorId, data.salesRepresentatorId, 'Sales Rep');
    ...
    htmlString=htmlString+createHistoryCompanies(company, oldRecord.companies, data.companies, 'Companies');
    
    oldRecord.currency!=data.currency?htmlString=htmlString + createHistoryFields(oldRecord.currency, data.currency, 'Currency'):null
